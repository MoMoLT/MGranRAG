...
def evaluate_metric_recall_k(example_gold_docs: List[str], example_relevant_docs: List[str], k_list = [1, 2, 5, 10, 20, 30, 50, 100, 150, 200]):
    example_eval_result = {}
    if len(example_relevant_docs) < k_list[-1]:
        logger.warning(
            f"Length of retrieved docs ({len(example_relevant_docs)}) is smaller than largest topk for recall score ({k_list[-1]})")

    for k in k_list:
        top_k_docs = example_relevant_docs[:k]
...
def evaluate_metric_extract_match(gold_list: List[str], prediction: str, aggregation_fn: Callable = np.max):
    em_scores = [1.0 if normalize_answer(gold) == normalize_answer(prediction) else 0.0 for gold in gold_list]
    aggregated_em = aggregation_fn(em_scores)
    return aggregated_em


def f1_score(gold_answer: str, prediction: str):
...
def batch_evaluate_metric_qa_em(gold_answers: List[List[str]], predicted_answers: List[str], aggregation_fn: Callable = np.max):
...
def batch_calculate_metric_qa_f1(gold_answers: List[List[str]], predicted_answers: List[str], aggregation_fn: Callable = np.max):
    """
